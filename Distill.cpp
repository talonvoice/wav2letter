...
int main(int argc, char** argv) {
  ...
  /* =========================== Load Big Model =========================== */
  LOG(INFO) << "[Network] Reading reference acoustic model from " << FLAGS_am;
  std::shared_ptr<fl::Module> refNetwork;
  std::shared_ptr<SequenceCriterion> refCriterion;
  std::unordered_map<std::string, std::string> refCfg;
  std::shared_ptr<fl::FirstOrderOptimizer> refCritoptim;
  W2lSerializer::load(FLAGS_am, refCfg, refNetwork, refCriterion, refCritoptim);
  // used for validation
  FLAGS_criterion = refCfg["criterion"];

  /* =========== Create Network & Optimizers / Reload Snapshot ============ */
  std::shared_ptr<fl::Module> network;
  std::shared_ptr<SequenceCriterion> skipCriterion;
  ...
  if (runStatus == kTrainMode) {
    ...
    W2lSerializer::load(
        reloadPath, cfg, network, skipCriterion, netoptim);
  }
  ...
  auto saveModels = [&](int iter) {
    if (isMaster) {
      ...
      if (FLAGS_itersave) {
        ...
        W2lSerializer::save(
            filename, config, network, refCriterion, netoptim, refCritoptim);
      }
      ...
      for (const auto& v : validminerrs) {
        ...
        if (verr < validminerrs[v.first]) {
          ...
          W2lSerializer::save(
              vfname, config, network, refCriterion, netoptim, refCritoptim);
        }
      }
    }
  };
  ...
  /* ===================== Hooks ===================== */
  auto evalOutput = [&dicts, &refCriterion](
                        const af::array& op,
    ...
    for (int b = 0; b < batchsz; ++b) {
      ...
      auto viterbipath =
          afToVector<int>(refCriterion->viterbiPath(op(af::span, af::span, b)));
  ...
  auto train = [&meters,
                ...
                &reducer](
                   std::shared_ptr<fl::Module> ntwrk,
                   std::shared_ptr<fl::Module> refNtwrk,
                   std::shared_ptr<SequenceCriterion> crit,
    ...
    fl::distributeModuleGrads(refNtwrk, reducer);
#endif
    ...
    fl::allReduceParameters(refNtwrk);
#endif
    ...
    auto runValAndSaveModel = [&](int64_t epoch, double lr) {
      meters.runtime.stop();
      meters.timer.stop();
    ...
    while (curEpoch < nepochs) {
      ...
      for (auto& sample : *trainset) {
        ...
        // cross-entropy loss
        auto loss = MSE.forward(output, refOutput);
        af::sync();
        ...
        if (FLAGS_reportiters > 0 && sampleIdx % FLAGS_reportiters == 0) {
          runValAndSaveModel(curEpoch, netopt->getLr());
          resetTimeStatMeters();
          ntwrk->train();
          meters.sampletimer.resume();
          meters.runtime.resume();
